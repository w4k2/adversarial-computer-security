...
def print_table(client, runs, experiment_id, num_tasks=None):
    ...
    tab_latex = tabulate.tabulate(table, tablefmt="latex", headers=['method', 'acc_first', 'acc', 'FM', ])
...
def calc_average_metrics(dataset_run_ids, client, experiment_id, num_tasks=None):
    ...
    for run_id in dataset_run_ids:
        acc, first_task_acc = get_metrics(run_id, client)
        acc_all.append(acc)
        acc_first_all.append(first_task_acc)
        fm = calc_forgetting_measure(run_id, client, experiment_id=experiment_id, num_tasks=num_tasks)
        fm_all.append(fm)

    acc = mean_std(acc_all)
    acc_first = mean_std(acc_first_all)
    std = mean_std(fm_all)
    return acc_first, acc, std

def mean_std(metric_all):
    avrg_metric = sum(metric_all) / len(metric_all)
    avrg_metric = round(avrg_metric, 4)
    metric_std = np.array(metric_all).std()
    metric_std = round(metric_std, 4)
    metric_str = f'{avrg_metric}±{metric_std}'
    return metric_str
